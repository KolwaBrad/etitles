            ...
            <table>
                ...
                <tbody> 
                        @foreach (session('alltransactions') as $transaction)
                            @if($transaction->admin_approve == "1" && $transaction->bidder_id == session('anyUserId') && is_null($transaction->bank_statement))
                            <tr>
                                ...
                                <td>
                                <form action="{{ route('addpayments') }}" method="POST" enctype="multipart/form-data">
                            @csrf
